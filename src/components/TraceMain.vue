            ...
            <el-col :span="24">
                ...
                <el-card v-bind:style="{display:dis1}">
                    ...
                    <el-table ref="classTable" class="table1" :data="tabledata" border @expand-change="handle_expand"
                        ...
                        <el-table-column type="expand">
                            <template slot-scope="props">
                                <el-table border :data="props.row.dicts" class="table2" :key="props.row.id"
                                    ...
                                    <el-table-column prop="LineID" label="产线ID" width="80px">
                                    </el-table-column>
                                    <el-table-column prop="TestResult" label="测试结果" width="90px">
                                    </el-table-column>
                                    ...
                                    <el-table-column :label="item.propName" :property="item.prop"
                                        v-for="item in props.row.colname" :key="item.prop" align="center">
                                        <template slot-scope="scope">
                        ...
                        <el-table-column label="Station" prop="StationDesc"></el-table-column>
                    </el-table>
                </el-card>
                ...
                if (sub == "WIB") {
                    ...
                    SearchInfo(params2)
                        .then(res => {
                            ...
                            for (var i = 0; i < res.data.total; i++) {
                                this.tablecolumn1.push({ prop: res.data.rows[i].componentitem, propName: res.data.rows[i].type2 });
                            }
                        })
                        .catch(function (error) { console.log(error) })
                }
            ...
            get_compopnrnt(row) {
                ...
                SearchInfo(params)
                    .then(rspData => { //这里是我们项目里封装的ajax请求方法，相当于axios.post() 
                        ...
                        var arr = [];
                        this.tablecolumn = rspData.data.rows;
                        if (row.stationid == "ST00") {
                            this.$set(rspData.data.rows, 0, { prop: "Parameter1", propName: "料号" })
                            this.$set(rspData.data.rows, 1, { prop: "WorkOrder", propName: "工單號" })
                        }
                        for (var i = 0; i < rspData.data.total; i++) {
                            this.$set(rspData.data.rows, i, { prop: rspData.data.rows[i].componentitem, propName: rspData.data.rows[i].type2 })
                        }
                        console.log(row.colname);
                        self.$set(self.tabledata[index], 'colname', rspData.data.rows) //这里就是给父表格数据数组self.pageData.results第index个对象加上dicts这个属性，然后把rspData.data我们从后台拿到的数据绑定到dicts这个key
                    }) //关键就是这个方法，row.id是父分组的id需要传给后台查询该子分组的信息 
                ...
                //             this.$set(this.tableColumnList, 1, { prop: "WorkOrder", propName: "工單號" })
                //             // this.$set(this.tableColumnList, 2,  { prop: "Parameter2", propName: "數量" })
                //         }
                //         for (var i = 0; i < res.data.total; i++) {
                ...
                //         }
                //         row.colname=this.tableColumnList;
                //         console.log(row.colname);
                ...
                if (sub == "WIB") {
                    ...
                    SearchInfo(params2)
                        .then(res => {
                            this.tablecolumn = res.data.rows;
                            for (var i = 0; i < res.data.total; i++) {
                                this.tablecolumn2.push({ prop: res.data.rows[i].componentitem, propName: res.data.rows[i].type2 });
                            }
                        })
                        .catch(function (error) { console.log(error) })
                }
